  ...
  return (
    <Modal
      ...
      onOk={() => {
        if (
          recordInfo.amount &&
          recordInfo.categoryId &&
          recordInfo.amount > 0
        ) {
          mode === 'create' && onCreate && onCreate(recordInfo)
          mode === 'edit' &&
            onEdit &&
            onEdit({ ...recordInfo, id: recordInfo.id })
    ...
    >
      <Col style={{ marginBottom: '15px' }}>
        <Select
          showSearch
          notFoundContent={
            <div>{`${categories?.length ? 'Упс :( Ничего такого у тебя нет' : 'Сначала нужно создать хотя бы одну категорию!'}`}</div>
          }
          allowClear
          ...
          filterOption={(input, option) =>
            (
              (option?.children as Omit<DefaultOptionType, 'children'>)?.props
                ?.children ?? ''
            )
              .toString()
              .toLowerCase()
          ...
          onChange={(value) =>
            onChangeRecordInfo((prev) => ({ ...prev, categoryId: value }))
          }
        >
          {categories.map((c) => (
      ...
      <Col>
        <InputNumber
          ...
          onChange={(value) =>
            onChangeRecordInfo((prev) => ({ ...prev, amount: value }) as any)
          }
        />
      </Col>
    </Modal>
