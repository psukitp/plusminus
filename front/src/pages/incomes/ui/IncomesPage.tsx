...
const IncomesPage = () => {
  ...
  const { addIncome, deleteIncome, editIncome, incomes, incomesLoading } = useIncome(
  ...
  return (
    <>
      <ExpenseIncomeView
        ...
        setPeriod={setPeriod}
      />
      <Edit id={editingId} onClose={() => setEditingId(null)} onEdit={(income) => editIncome({ id: editingId!, ...income })} />
      <Delete id={deletingId} onClose={() => setDeletingId(null)} onDelete={(id) => deleteIncome(id)} />
      <Create open={isCreating} onClose={() => setIsCreating(false)} onCreate={(income) => addIncome(income)} />
    </>
  )
}
